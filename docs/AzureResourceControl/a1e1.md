---
Atelier: Utilisation de l'Azure Resource Control pour gérer une machine Windows depuis l'environnement Azure
Duree: 30
---
...
> La machine créée ici simulera une machine Windows qui ne serait pas hébergée dans Azure  
...
> Vous avez besoin d'attendre que le déploiement de la machine virtuelle soit terminé pour passer à l'étape suivante.  
...
1. Une fois la session ouverte, attendez, si nécessaire, qu'elle s'initialise correctement.
  ...
  > Dans la session distante, si le panneau **Networks** s'affiche, cliquez sur **yes**.
...
> Vous pouvez vérifier si vous le souhaitez que, dans le portail azure, la machine virtuelle ne communique plus avec l'environnement Azure car un message apparait sur la resource VM dans le portail indiquant "**ARC-VM virtual machine agent status is not ready.**"  
