# Mise en oeuvre de la synchronisation d'identités
...
1. Si besoin, dans la boite **Sign in**, utilisez l'adresse de connexion de Dominique Skyetson (```dom@[onmicrosoftDomain].onmicrosoft.com```) et cliquez sur **Next**.
	...
	1. Dans la boite **Enter password**, saisissez ```[userPass]``` et cliquez sur **Sign in**.
...
1. Connectez vous ensuite avec le compte de Dominique (mot de passe ```[userPass]```).
...
1. Connectez vous avec le compte de Dominique (```dom@[onmicrosoftDomain].onmicrosoft.com``` avec son mot de passe ```[userPass]```.
...
1. Dans la fenêtre **Sign in** qui apparaît, connectez-vous avec le compte de Dominique Skyetson : ```dom@[onmicrosoftDomain].onmicrosoft.com``` (ou sélectionnez le dans la fenêtre **Pick an Account** le cas échéant) et son mot de passe (*[userPass]*). 
