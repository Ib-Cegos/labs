...
1. Dans la fenêtre **Sign-in**, entrez ```Alan@[onmicrosoftDomain].onmicrosoft.com```.  
1. Dans la fenêtre **Enter password**, tapez ```[defaultPass]```.
1. Dans la fenêtre **Update your pasword**, changez le mot de passe de Alan comme déjà vu précédemment de **Pa55w.rd** vers ```[userPass]```.
...
1. Dans la fenêtre **Enter password**, entrez ```[defaultPass]```.
...
1. Dans le panneau **Reset password** de Elvis, décochez la case **Automatically create a password**, et saisissez **```[defaultPass]```** dans le champ **Password**. Si nécessaire, cochez la case **Require this user to change their password when they first sign in**.
...
1. Dans le panneau **Reset password** pour Alan, si nécessaire, décochez la case **Automatically create a password** et saisissez ```[defaultPass]``` dans le champ **Password**. Si nécessaire,  sélectionnez la case à cocher **Require this user to change their password when they first sign in**.
1. Cliquez sur **Reset password**.
...
1. Sur la page **Sign in**, vérifiez l'adresse saisie pour Leila avant de cliquer sur **Next** et saisir le mot de passe ```[defaultPass]```.
