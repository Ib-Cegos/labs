# Activation de la jonction de domaine hybride
...
1. Sur la page **Connect to Entra**, saisissez les informations de connexion de Dominique Skyetson (```dom@[onmicrosoftDomain].onmicrosoft.com``` dans le champ **USERNAME** et ```[userPass]``` dans le champ **PASSWORD**) puis cliquez sur **Next**.
