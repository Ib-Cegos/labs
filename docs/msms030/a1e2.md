...
1. Retournez dans l'onglet **DNS** de votre environnement d'atelier et créez-y tous les enregistrements DNS nécessaires pour le tenant du projet pilote.
    ...
    > Vous pouvez voir un exemple de résultat attendu à l'issue de cette opération dans l'illustration disponible sur le coté de cet exercice : il vous faudra cependant remplacer la mention labXXXXXX par votre nom DNS d'entreprise ([godeployDomain]).
