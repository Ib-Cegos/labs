# Gestion des utilisateurs et des groupes avec Windows PowerShell
...
1. Dans la partie basse (fond bleu) de la fenêtre **Administrator: Windows PowerShell ISE**, tapez `install-module microsoft.graph -force` et faites **[Entrée]** si nécessaire.
	...
	> Si le système Windows n'est pas correctement à jour dans la machine virtuelle, réalisez ces opérations préparatoires dans une invite **Windows Powershell** standard (en fermant la fenêtre **Windows Powershell ISE** pour la réouvrir ensuite à partir de la tâche 2...)

	> Notez également que l'installation du module Microsoft Graph est **particulièrement longue**... patience donc...
...
1. Dans la partie basse (fond bleu) de l'outil, tapez la commande suivante : 
```powershell
...
> Si vous rencontrez un problème sur cette commande avec l'éditeur Powershell ISE, vous pouvez raliser ces manipulations dans l'outil ligne de commande "**Windows Powershell**".  
...
1. la commande suivante va créer un second compte utilisateur pour **Tameka Reed**:
	...
	> Vous pouvez simplement taper la commande ```$user2``` pour afficher le résultat de l'opération précédente avant de passer à la suite.
...
1. Utilisez la commande suivante pour obtenir la licence **Microsoft 365 E5** disponible dans le contexte du projet pilote :
	...
	> Vous pouvez simplement taper la commande `$license` pour afficher le résultat de l'opération précédente avant de passer à la suite.
...
1. Laissez l'outil **Windows Powershell ISE** (ou **Windows Powershell**) ouvert pour l'utiliser de nouveau dans la tâche suivante.
...
1. Retournez à **Administrator : Windows Powershell ISE** (ou **Administrator: Windows Powershell**) pour utiliser la commande suivante pour procéder à l'import des utilisateurs contenus dans le fichier :
	```powershell
...
1. Minimiser l'outil **Administrator : Windows Powershell ISE** (ou **Administrator: Windows Powershell**) et retournez dans votre navigateur Internet. 
...
1. Conservez la session ouverte sur la machine virtuelle LON-CL1, réduisez l'outil **Administrator : Windows Powershell ISE** (ou **Administrator : Windows Powershell**) dans la barre des tâches et maximisez la fenêtre de votre navigateur Internet pour l'exercice suivant.
